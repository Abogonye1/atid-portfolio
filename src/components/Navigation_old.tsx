...
const Navigation = () => {
  ...
  const scrollToId = (id: string, opts?: { duration?: number; offset?: number; highlight?: boolean }) => {
    const el = document.getElementById(id);
    if (!el) return;
    const duration = Math.min(Math.max(opts?.duration ?? 400, 300), 500);
    ...
    const step = (now: number) => {
      ...
      if (t < 1) {
        ...
        // Temporary highlight on arrival
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (href === "#projects" || href === "#services") {
      e.preventDefault();
      const id = href.slice(1);
      scrollToId(id, { duration: 400, offset: 0, highlight: true });
      try {
  ...
  return (
    <nav className="w-full h-16 sm:h-20 md:h-24 flex items-center justify-between px-4 sm:px-6 lg:px-12 xl:px-16 2xl:px-24 relative z-50 max-w-screen-2xl mx-auto">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 lg:hidden">
        {/* Mobile menu trigger */}
        <div>
        <Sheet onOpenChange={(open) => setMenuOpen(open)}>
